...
ESD: the **7 keypad buttons** and **touch INT** (→ MCP23017 **U51 @0x21** Port A), and the
**enclosure tamper switch** (→ STM32 **PC13 / TAMP**, backup domain). Three TVS arrays cover the
three cable entries.

Designator note: the schematic labels the button/INA expander **U51**; older docs call it **U48**.
...
|---|---|---|---|---|---|
| Buttons | 7 | J9 (1×08) | U62 (4 ch) + U63 (3 ch) — TPD4E02B04DQAR | 10 kΩ→3V3_STM + 100 nF | U51 GPA0–GPA6 |
| Tamper | 1 | J8 (1×02) | U63 ch4 — TPD4E02B04DQAR | 1 nF, **no external PU** | PC13 / TAMP |
| Touch INT | 1 | J3 pin 13 (display flex) | **U17 spare ch — TPD4E05U06DQAR** | 10 kΩ→3V3_STM, **no cap** | U51 GPA7 |
...
## 2. Buttons — U51 GPA0–GPA6

```
[Panel SPST]──J9──TVS(U62/U63)──●node●──GPAn
                                │      │
                        10k→3V3_STM  100nF→GND
```

| Net | TVS pin | Pull-up | Debounce C | GPA / U51 pin |
|---|---|---|---|---|
| BUTTON_1 | U62 D1+ (p1) | R200 10 kΩ | C170 0.1 µF | GPA0 / 17 |
| BUTTON_2 | U62 D1− (p2) | R201 | C171 | GPA1 / 18 |
| BUTTON_3 | U62 D2+ (p4) | R202 | C172 | GPA2 / 19 |
| BUTTON_4 | U62 D2− (p5) | R203 | C173 | GPA3 / 20 |
| BUTTON_5 | U63 D1+ (p1) | R204 | C174 | GPA4 / 21 |
| BUTTON_6 | U63 D1− (p2) | R205 | C175 | GPA5 / 22 |
| BUTTON_7 | U63 D2+ (p4) | R206 | C176 | GPA6 / 23 |
...
```
[Lid SPST, NC]──J8──TVS(U63 ch4)──●RTC_TAMPER●──PC13
                                  │
                              C177 1nF→GND   (NO external pull-up)
```

| Element | Value | Purpose |
|---|---|---|
| TVS | U63 D2− (p5), TPD4E02B04DQAR | Enclosure-cable ESD |
| Cap | C177 **1 nF** | Holds the line between TAMP precharge samples; debounce |
| Pull-up | **none external** | Provided by the TAMP internal precharge pull-up (backup-domain powered) |
| Connector | J8 pin 1 = RTC_TAMPER, pin 2 = GND | Lid/intrusion contact across 1–2 |
...
## 4. Touch INT — U51 GPA7

```
FT6336U INT──J3.13──TVS(U17 spare ch)──●DISP_TOUCH_INT●──GPA7
...
| Pin | U51 GPA7 / 24; FT6336U INT arrives on J3 pin 13 | On-change → INTA → BTN_INT_N |

- **3.3 V line.** Although touch I²C (CTP_SCL/SDA, J3 p10/p12) idles at 5 V behind the PCA9306, the
  INT (J3 p13) is a **3.3 V** signal direct from the FT6336U (not through the module's 74LVC245). It
...
- **Pull-up rail rationale:** on 3V3_STM (always-on) so the line is defined high when the display
  rail (V_DISP_5V, gated by DISP_EN) is off — a gated-rail pull-up would let GPA7 float into a
...
|---|---|---|---|---|---|---|
| U62, U63 | TPD4E02B04DQAR | ±3.6 V | 5.5 V | ~8.8 V @ Ipp | 0.25 pF | 7 buttons + tamper (3.3 V logic) |
...
### 6.1 MCP23017 U51 @0x21 (Port A = buttons + touch INT)

- **MIRROR = 0** (Port A and Port B INTs separate). Port A INTA → `BTN_INT_N` → PE0/EXTI0
  (R148 10 kΩ pull-up). Port B INTB → `INA_ALERT_INT_N` → PC12/EXTI12 (R153).
...
- **Shared reset:** `EXP_RESET` (PC0, R154 pull-down) resets U47+U51; re-init both after any pulse.
...
- **Touch bring-up sequencing:** on DISP_EN enable, allow rail soft-start, release DISP_RST (PA8
  high) past the FT6336U reset margin, then I²C-init touch. Mask `V_DISP_EN_FAULT` (U47 GPB2) for the
...
- I²C wedge recovery: toggle `I2C_BUF_EN` (PA9, LTC4311 EN); if persistent, pulse `EXP_RESET` (PC0)
  to hard-reset U47/U51. Housekeeping is on always-on 3V3_STM, so there is no rail-cycle path — the
...
| Element | Purpose |
|---|---|
| 10 kΩ pull-up (R200–R206, R208) | Define idle-high; set active-low sense |
| 100 nF (C170–C176) | 1 ms RC button debounce |
| 1 nF (C177) | Precharge-compatible tamper hold/debounce |
| U62/U63 TPD4E02B04 | Connector ESD, 3.3 V-class (low clamp) for buttons + tamper |
| U17 TPD4E05U06 spare ch | Connector ESD for the 3.3 V touch INT, reusing a 5.5 V array's free channel |
| R148 (BTN_INT_N PU) | Single pull-up on the open-drain INTA net to PE0 |
| EXP_RESET / R154 | Shared expander reset, default-asserted via pull-down |
...
1. **U17 ESD locality (priority).** ESD must clamp at the cable entry. DISP_TOUCH_INT enters at the
   display flex **J3 pin 13** — confirm **U17 sits at J3**. If U17 is a remote array (e.g., the USB
   connector), a J3 strike propagates across the board before clamping and the protection is
   ineffective; in that case keep the INT on the local display array (U60 has spare channels) or add
   a local single-channel TVS at J3. **Confirm U17's location and which of its channels DISP_TOUCH_INT
   uses.**
2. **U60 channel count.** If DISP_TOUCH_INT moved off U60, U60 now carries only the two 5 V touch
...
6. **Designator/doc sync:** U51↔U48, R208↔R167; R207 gap (confirm used elsewhere); A0→VS / A1,A2→GND
   strap for 0x21; INTA(R148)/INTB(R153) pull-ups and U51 VDD=3V3_STM + 0.1 µF — all off-crop, verify
   on the full sheet.
