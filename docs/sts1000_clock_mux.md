...
- **One control line:** `MUX_SEL` = **PB6**, GPIO push-pull. **0 = OCXO (A, boot default)**, **1 = external/Rb (B)**. A **100 kΩ pull-down (R188)** on the SEL pin forces **A (OCXO)** before firmware runs (PB6 is Hi-Z at reset). Polarity matches the 74LVC1G157 function table directly (S=L→I0, S=H→I1) — no inversion.
...
- **Rail discipline:** the mux/buffer run on **3V3_CLK = the MCU VDD domain** (ferrite-isolated via FB10), **never** the 3.327 V OCXO LDO. PH0 abs-max is VDD+0.3; a >VDD Voh would over-drive it.
- **Always-on:** both references are permanently powered and present at A and B at all times (§2.1). The mux only *forwards* one; it never powers a source down. The 74LVC1G157 **IOFF** feature also prevents a live source back-powering the part if 3V3_CLK is down.
...
### 3.1 Selector — `U52` (74LVC1G157GW-Q100H)
...
### 3.4 Select default — `R188`
| Item | Value | Notes |
|---|---|---|
| `R188` | 100 kΩ 1%, `MUX_SEL` → GND | Forces S=0 (OCXO/A) at power-up/reset before PB6 is driven (PB6 resets Hi-Z). When firmware drives PB6 high (select B), it sources only 33 µA into R188 — negligible. SEL is static (set on a source change), so no series damping needed. |

### 3.5 Supply island — `FB10`, decoupling
| Item | Value | Notes |
|---|---|---|
| `FB10` | ferrite ≈ 600 Ω @ 100 MHz, 0603 | `3V3 → 3V3_CLK`. Keeps digital/SMPS spurs off the clock rail while staying in the VDD domain (so Voh ≤ VDD at PH0). |
| `C157` | 1 µF X7R 0402 | bulk on 3V3_CLK at FB10 output. |
| `C158` | 0.1 µF 0402 | at U52 VCC (pin 5), tight. |
| `C159` | 0.1 µF 0402 | at U53 VCC (pin 5), tight. |
...
|---|---|---|---|
| `MUX_SEL` | PB6 | GPIO out, push-pull, low speed | 0 = OCXO (A, boot), 1 = ext/Rb (B). Owned by the reference state machine. R188 → default A. |
...
|---|---|---|---|
| U52 | 74LVC1G157GW-Q100H | TSSOP6 (SOT363-2) | 2:1 clock selector; Schmitt in, IOFF |
| U53 | 74LVC1G34GW-Q100 | TSSOP5 (SOT353-1) | bench-fanout buffer |
| R188 | 100 kΩ 1% | 0402 | MUX_SEL pull-down → default OCXO |
| R187 | 22 Ω 1% | 0402 | Y → PH0 (`CLK_OUT`) series damp |
| R189 | 22 Ω 1% (trim 22–33) | 0402 | 50 Ω source-match, fanout |
| C158 | 0.1 µF | 0402 | U52 VCC decoupling (pin 5) |
...
## 8. Bring-up & test
1. **Rail:** 3V3_CLK = 3.30 ± 0.1 V; ripple low after FB10.
2. **Default select (no firmware):** PB6 Hi-Z → R188 holds S=0 → confirm `CLK_OUT` = OCXO 10 MHz at PH0, full 3.3 V swing, ~50 % duty. Verify VIH/VIL margins against 0.7/0.3·VDD.
...
4. **Bench fanout:** terminate `10MHz_RF_OUT` in 50 Ω; confirm ~1.5–1.65 Vpp, in phase with the active source; trim R189 if amplitude/match needs it.
...
|---|---|---|
| Boots on B (ext) instead of A | R188 open / wrong-variant part ('158) | meter S at power-up = 0 V; confirm part is '157 |
| PH0 not toggling / HSE won't ready | U52 Y dead, R187 open, wrong rail | scope Y node and `CLK_OUT`; confirm 3V3_CLK; confirm input present at I0 |
| HSE marginal / intermittent HSERDY | Voh < 0.7·VDD (rail sag) or duty out of band | confirm 3V3_CLK ≥ 3.3 V; check source duty |
...
| Fanout wrong amplitude/reflections | R189 mismatch, far-end not 50 Ω, C160 open | trim R189; verify termination |

-----

## 10. Open items
1. Trim R187 / R189 against the actual `CLK_OUT` and fanout trace lengths post-layout.
2. Firmware: minimize the CSS masking window in §4.2 — don't blind the failsafe longer than the switch.
3. Confirm bench-equipment 10 MHz input impedance / level expectation to finalize R189 (and whether DC-block C160 alone suffices).
4. *(Optional)* if bench-fanout phase noise is ever found marginal, swap U53 → a dedicated low-additive-jitter clock buffer; for U52, 74AUP1G157 is a lower-noise drop-in. Interface unchanged.
